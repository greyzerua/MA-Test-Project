...
const getImgSlider = (images) => {
  ...
  productImgSliderPrev.classList.add(
    "product-item__img-slider-btn",
    "product-item__img-slider-prev"
  );
  productImgSliderPrev.innerHTML = "<";

  const productImgSliderNext = document.createElement("div");
  productImgSliderNext.classList.add(
    "product-item__img-slider-btn",
    "product-item__img-slider-next"
  );
  productImgSliderNext.innerHTML = ">";

  productImgSliderPrev.addEventListener("click", () => {
    sliderImagesEl[activeSlideIndex].classList.remove(
      "product-item__img-slide_active"
    );

    activeSlideIndex =
      activeSlideIndex === 0 ? sliderImagesEl.length - 1 : --activeSlideIndex;
    sliderImagesEl[activeSlideIndex].classList.add(
      "product-item__img-slide_active"
    );
  });
  ...
  productImgSliderNext.addEventListener("click", () => {
    ...
    activeSlideIndex =
      activeSlideIndex === sliderImagesEl.length - 1 ? 0 : ++activeSlideIndex;
    sliderImagesEl[activeSlideIndex].classList.add(
      "product-item__img-slide_active"
    );
  });

  productImgContainer.append(
    productImgSlider,
    productImgSliderPrev,
    productImgSliderNext
  );
...
const getLikeButton = (productId) => {
  ...
  productLikeBtn.style.backgroundImage = isLiked
    ? "url(./assets/heart-filled.png)"
...
const renderLoader = (container, loading) => {
  container.innerHTML = loading
    ? '<div class="products-section__loader"><img src="./assets/loader.svg"/></div>'
    : null;
};

const renderError = (container, error) => {
  container.innerHTML = error
    ? `<div class="products-section__error">${error}</div>`
    : null;
};
